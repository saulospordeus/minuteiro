...
`minuteiro` is a multi-page Phoenix application for creating, editing, filling, and compiling legal document templates with data persistence and optional AI-assisted generation.

## Current project state

- Phoenix 1.8 base application is already bootstrapped in this repository
...
- The core of the product is the template language engine, not the LiveView layer
- `Minuteiro.Parser` is responsible for extracting variables, references, and conditional blocks
- `Minuteiro.Compiler` is responsible for evaluating conditions, injecting answers, and producing the final clean document
- LiveView should stay thin and only orchestrate UI state, user events, and persistence

### Planned web layer

- `/` should show a dashboard with cards listing saved templates
- `/templates/:id/edit` should host a dedicated `TemplateEditorLive`
- The editor socket should keep only:
  - `template`
  - `variables`
  - `answers`
  - `final_document`
  - minimal async AI state

### Planned data layer

- `Minuteiro.Documents.Template`
  - `title :string`
  - `content :text`
  - `description :text`
  - `timestamps()`
- Future evolution must remain easy for:
  - `template_versions`
  - `generated_documents`
